...
- Real LLM: configure `llm.providers[]` in `storage/config.json` (copy from `storage/config.example.json`) and set `llm.stub=false` (or omit stub once a key exists). Legacy env (`BP_LLM_PROVIDERS` + `BP_LLM_<ID>_*`) still works as bootstrap before the file exists.
- **Source of truth:** `storage/config.json` (gitignored) holds limits, `llm.*` globals (strategy/providers/stream/vision/effort/circuit/retry backoff), `context.*`, `docs.*`, `web_search.*`, `skills_root`, and `mcp.*`. Env (`BP_*`) is bootstrap: when a JSON key is omitted, the env-derived default wins. Env-only: `BP_HTTP_ADDR`, `BP_WEB_ROOT`, `BP_STORAGE_ROOT`, `BP_DOCS_ROOT`, `BP_PROMPTS_ROOT`, `BP_TOOLS_ROOT`, `BP_CONFIG_PATH`, `BP_LLM_RETRY_STATUSES`, `BP_LLM_STUB`. See [`docs/architecture/settings-config.md`](docs/architecture/settings-config.md).
- `llm.providers[].api=responses` (preferred, AIPedia-aligned) or `chat`. Default `llm.stream=true` (or env `BP_LLM_STREAM`): client sends `stream=true` and parses proxy SSE (`text/event-stream`); JSON non-stream remains a path. Set `llm.stream=false` to force JSON. If streaming is rejected as unsupported, client retries once with `stream=false`.
...
- Settings UI + JSON config: `storage/config.json` (gitignored; see `storage/config.example.json`), env bootstrap then file SoT — [`docs/architecture/settings-config.md`](docs/architecture/settings-config.md). API under `/api/settings`.
