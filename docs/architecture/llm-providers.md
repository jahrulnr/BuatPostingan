...
| Mode | When | Behavior |
|---|---|---|
| **Stub** | `BP_LLM_STUB` / `WEBCHAT_LLM_STUB` = true, **or** unset and **no** provider `API_KEY` | Worker skips HTTP; appends canned `agent_message` + `turn.completed` |
...
Providers are **named slots**, not hardcoded vendor packages. `BP_LLM_PROVIDERS` / `WEBCHAT_LLM_PROVIDERS` is a comma list of IDs (default `OPENROUTER`). For each `ID`:

| Env (product / portable) | Meaning |
|---|---|
| `BP_LLM_<ID>_BASE_URL` / `WEBCHAT_LLM_<ID>_BASE_URL` | Base URL without trailing slash (client appends path) |
...
- Body: `messages`, `tools` (function shape), `tool_choice=auto`, `max_tokens`, **`stream=true`**
...
- Tools as Responses `function` objects; **`stream=true`**
...
- Non-stream JSON remains a **fallback** only

Do not force `stream=false` / `API=chat` just to get JSON — some local proxies degrade Responses to empty chat.completion when streaming is disabled.
...
- [Portable AI kit](portable-ai-kit.md) — `WEBCHAT_LLM_*` aliases for embeds
